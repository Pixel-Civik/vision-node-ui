...
def build_sidebar_filters(sites, channels, zones, events):
    ...
    sel_events = st.multiselect("Tipo de Evento", options=events, default=[e for e in events if e in ("enter", "exit")] or events)
    show_presence = st.toggle("Incluir eventos de presencia", value=False)
    hour_min, hour_max = st.slider("Rango Horario", min_value=0, max_value=23, value=(0, 23))
    ...
    return sel_sites, sel_channels, sel_zones, sel_events, show_presence, hour_min, hour_max, dow_sel, align_series, metric_mode
