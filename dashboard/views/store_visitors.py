...
def render_store_visitors(f: pd.DataFrame, ctx: dict) -> None:
    ...
    base = base[base["event_type"] == "visitor"].copy()
    if base.empty:
        st.info("No hay eventos de visitantes (visitor) con los filtros seleccionados.")
        return
    ...
    if zones:
        ...
        zone_sel = st.selectbox("Zona", options=zone_options, index=0, key="visitor_zone_sel")
        if zone_sel != "Todas":
    ...
    c2.metric("Promedio Visitantes/Día", f"{avg_per_day}")
    ...
    if not by_hour.empty:
        ...
        if not peak_row.empty:
            ...
            st.caption(f"Pico horario (Lima): {peak_hour:02d}:00 con {peak_count} visitantes.")
    ...
    if zone_col and zones:
        ...
        ch_zone = (
            ...
            .encode(
                ...
                x=alt.X("count:Q", title="Visitantes"),
